...
contract coin {
    ...
    modifier noWallet(){
        require(!ColoredWallets[msg.sender].initialized, "Your wallet already initialiezed!");
        _;
    }
    ...
    modifier checkMinimumBlueCoin(uint blueCoin){
        require(blueCoin < ColoredWallets[msg.sender].numberOfBlueCoins , "You don't have enough blue conis!");
        _;
    }

    function transfer (uint256 _numberOfBlueCoins, address receiver) public checkMinimumBlueCoin(_numberOfBlueCoins) checkReceive(receiver) checkRecierWallet(receiver) {
        ColoredWallet memory wallet = ColoredWallets[msg.sender];
        wallet.numberOfBlueCoins = wallet.numberOfBlueCoins - _numberOfBlueCoins;
        ColoredWallets[msg.sender] = wallet;
        ColoredWallet memory receiverWallet = ColoredWallets[receiver];
        receiverWallet.numberOfBlueCoins = receiverWallet.numberOfBlueCoins + _numberOfBlueCoins;
        ColoredWallets[receiver] = receiverWallet;
    }

    modifier checkReceive(address receiver){
         require(!ColoredWallets[receiver].initialized, "Receiver does not have wallet!");
        _;
    }

    modifier checkRecierWallet (address receiver) {
        require(msg.sender != receiver, "Sender and receiver have the same address!");
        _;
    }
