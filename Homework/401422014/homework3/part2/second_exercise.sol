...
contract secondExercise {
    ...
    function checkColoredWlletisExist (address _walletAddress) private view {
        require(isColoredWalletExists[_walletAddress] == false,"Wallet with this address already exists.");
    }

    function checkColoredWlletisNotExist (address _walletAddress) private view  {
    ...
    function checkWalletBlucCoinBalance  (uint256 _amount) private view{
    ...
    function createColoredWallet () public {
        checkColoredWlletisExist(msg.sender);
    ...
    function getWalletNumberOfRedCoins() public view returns(uint256){
        checkColoredWlletisNotExist(msg.sender);
        return usersWallet[msg.sender].numberOfRedCoins;
    }

    function getWalletNumberOfBlueCoins() public view returns(uint256){
        checkColoredWlletisNotExist(msg.sender);
    ...
    function convertBlueCoinToRedCoin() public{
        checkColoredWlletisNotExist(msg.sender);
        checkWalletBlucCoinBalance(0);
        usersWallet[msg.sender].numberOfBlueCoins -= 1;
        usersWallet[msg.sender].numberOfRedCoins += 2;
    }

    function sendBlueCoinToOthers(uint256 _sendingAmount , address _destinationWallet) public{
        checkColoredWlletisNotExist(msg.sender);
        checkColoredWlletisNotExist(_destinationWallet);
        checkWalletBlucCoinBalance(_sendingAmount);
